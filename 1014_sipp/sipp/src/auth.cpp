...
static void hex_to_bytes(u_char* bytes, const char* hex_str, int len) {
    for (int i = 0; i < len; i++) {
    ...
    unsigned int nonce_count, char* result, size_t result_len)
{
    ...
    char newUri[MAX_HEADER_LEN];
    // Extract the  realm
    if ((start = stristr(auth, "realm=")) == nullptr) {
        TRACE_CALLDEBUG("错误: 无法解析realm值\n");
        ERROR("无法解析realm值");
        snprintf(result, result_len, "createAuthHeaderAKAv1MD5: couldn't parse realm");
        return 0;
    }
    ...
    strncpy(newUri, "sip:", sizeof(newUri));
    strncpy(newUri + strlen(newUri), start, end - start);
    newUri[end - start] ='\0';
    // Extract the Nonce
    ...
    // memcpy(k, aka_K, KLEN);
    hex_to_bytes(k, aka_K, KLEN);
    for(i=0; i < KLEN; i++) {
        sprintf(debug_hex+i*2, "%02x", k[i]);
    }
    // TRACE_CALLDEBUG("计算前的k: %s\n", debug_hex);
    // memcpy(amf, aka_AMF, AMFLEN);
    hex_to_bytes(amf, aka_AMF, AMFLEN);
    for(i=0; i < AMFLEN; i++) {
        sprintf(debug_hex+i*2, "%02x", amf[i]);
    }
    // TRACE_CALLDEBUG("计算前的amf: %s\n", debug_hex);
    // memcpy(op, aka_OP, OPLEN);
    hex_to_bytes(op, aka_OP, OPLEN);
    for(i=0; i < OPLEN; i++) {
        sprintf(debug_hex+i*2, "%02x", op[i]);
    }
    // TRACE_CALLDEBUG("计算前的op: %s\n", debug_hex);



    // 打印RAND
    // for(i=0; i<RANDLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", rnd[i]);
    // }
    // TRACE_CALLDEBUG("RAND: %s\n", debug_hex);

    // 打印AUTN参数
    // for(i=0; i<SQNLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", sqnxoraka[i]);
    // }
    // TRACE_CALLDEBUG("SQN⊕AK: %s\n", debug_hex);

    // for(i=0; i<MACLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", mac[i]);
    // }
    // TRACE_CALLDEBUG("MAC: %s\n", debug_hex);
    ...
    // // 打印计算结果
    // for(i=0; i<RESLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", res[i]);
    // }
    // TRACE_CALLDEBUG("计算得到的RES: %s\n", debug_hex);

    // for(i=0; i<CKLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", ck[i]);
    // }
    // TRACE_CALLDEBUG("计算得到的CK: %s\n", debug_hex);

    // for(i=0; i<IKLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", ik[i]);
    // }
    // TRACE_CALLDEBUG("计算得到的IK: %s\n", debug_hex);

    // for(i=0; i<AKLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", ak[i]);
    // }
    // TRACE_CALLDEBUG("计算得到的AK: %s\n", debug_hex);
    ...
    // // 打印解密后的SQN
    // for(i=0; i<SQNLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", sqn[i]);
    // }
    // TRACE_CALLDEBUG("解密得到的SQN: %s\n", debug_hex);
    /* compute XMAC */
    f1(k, rnd, sqn, amf, xmac, op);
    // // 打印计算得到的XMAC
    // for(i=0; i<MACLEN; i++) {
    //     sprintf(debug_hex+i*2, "%02x", xmac[i]);
    // }
    // TRACE_CALLDEBUG("计算得到的XMAC: %s\n", debug_hex);
    ...
    if (1/*sqn[5] > sqn_he[5]*/) {
        ...
        written = createAuthHeaderMD5(
            user, (const char *)res, RESLEN, method, newUri, msgbody, auth,
            algo, nonce_count, result, result_len);
        if (written == 0) {
            free(nonce);
            snprintf(
        ...
        written = createAuthHeaderMD5(
            user, "", 0, method, newUri, msgbody, auth, algo, nonce_count,
            result, result_len);
        if (written == 0) {
            free(nonce);
            snprintf(
