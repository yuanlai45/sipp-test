...
usage() {
  ...
  --initial-imsi <号码>  起始IMSI号码 (默认: 460119000000000)
...
start_sipp_instance() {
    ...
    # 计算合适的max_socket值
    local max_socket_value=$((users + 100))  # 用户数 + 100个缓冲socket
    
    # 使用prlimit设置文件描述符限制
    echo "使用prlimit设置文件描述符限制到10000..."
    prlimit --nofile=10000:10000 --pid=$$ 2>/dev/null || {
        echo "警告: 无法设置文件描述符限制，尝试使用sudo..."
        sudo prlimit --nofile=10000:10000 --pid=$$ 2>/dev/null || echo "警告: 设置文件描述符限制失败"
    }
...
main() {
    # 初始化日志和错误统计
    init_logging "test_suite/logs/register_test"
    init_error_stats
    ...
    # 创建必要的目录
    mkdir -p test_suite/{logs/register_test,config,pids}
    
    # 确保清理现有环境
    cleanup
    
    # 创建用户配置
    create_csv "test_suite/config/users_0.csv" $USERS_PER_INSTANCE "$SCENARIO" "$INITIAL_IMSI" "$INITIAL_MSISDN" || {
