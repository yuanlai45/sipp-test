...
int sipp_bind_socket(SIPpSocket *socket, struct sockaddr_storage *saddr, int *port)
{
    ...
    // If multisocket is enabled and we have a user port specified
    if (multisocket && user_port > 0) {
        // Initialize next_socket_port if not set
        if (next_socket_port == 0) {
            next_socket_port = user_port;
        }
        
        // Try binding to next_socket_port
        sockaddr_update_port(saddr, next_socket_port);
        ret = ::bind(socket->ss_fd, (sockaddr *)saddr, len);
        
        if (ret == 0) {
            // Bind successful, increment port for next socket
            if (port) {
                *port = next_socket_port;
                socket->ss_port = next_socket_port;
            }
            next_socket_port++;
            return 0;
        }
    ...
    // Fall back to original binding logic
...
void sipp_customize_socket(SIPpSocket *socket)
{
    ...
    /* Set SO_REUSEADDR and SO_REUSEPORT for all sockets */
...
#ifdef SO_REUSEPORT
    if (setsockopt(socket->ss_fd, SOL_SOCKET, SO_REUSEPORT, (void *)&sock_opt,
                   sizeof (sock_opt)) == -1) {
        ERROR_NO("setsockopt(SO_REUSEPORT) failed");
    }
#endif

    /* Additional TCP/TLS/SCTP specific settings */
...
TEST(get_trimmed_call_id, withslashes) {
    EXPECT_STREQ("abc2", get_trimmed_call_class("OPTIONS..\r\nBla: X\r\nCall-ID: ///abc2\r\nCall-ID: def\r\n\r\n"));
    EXPECT_STREQ("abc3", get_trimmed_call_class("OPTIONS..\r\nBla: X\r\nCall-ID: abc2///abc3\r\nCall-ID: def\r\n\r\n"));
    EXPECT_STREQ("abc4///abc5", get_trimmed_call_class("OPTIONS..\r\nBla: X\r\nCall-ID: abc3///abc4///abc5\r\nCall-ID: def\r\n\r\n"));
}
